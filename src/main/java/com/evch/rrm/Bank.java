...
public class Bank {
    ...
    private Runnable transferFunds() {
        return new Runnable() {
            @Override
            public void run() {
                int to, from = Bank.this.pickRandomAccountId();
                do {
                    to = Bank.this.pickRandomAccountId();
                } while (to == from);
                if (Bank.this.getAccountBalance(from) > 0) {
                    long x = new Random().nextLong(Bank.this.getAccountBalance(from) + 1);
            ...
            private void transferFunds(int from, int to, long x) {
                // withdraw
                long balFrom = Bank.this.getAccountBalance(from) - x;
                Bank.this.setAccountBalance(from, balFrom);
                // deposit
                long balTo = Bank.this.getAccountBalance(to) + x;
                Bank.this.setAccountBalance(to, balTo);
            }
        };
    }
